...
class Subscription(models.Model):
    ...
    subtitle = models.TextField(max_length=120, blank=True, null=True)
    ...
    def get_features_as_list(self):
        if not self.features:
            return []  # return empty list if no features are defined
...
class SubscriptionPrice(models.Model):
    ...
    def stripe_currency(self):
        return "usd"  # Change this to change currency
...
class UserSubsciptionQuerySet(models.QuerySet):
    ...
    def by_user_ids(self, user_ids=None):
        ...
        return self
class UserSubsciptionManager(models.Manager):
    def get_queryset(self):
        return UserSubsciptionQuerySet(self.model, using=self._db)
...
class UserSubscription(models.Model):
    ...
    stripe_id = models.CharField(max_length=120, blank=True, null=True)
    active = models.BooleanField(default=True)
    user_cancelled = models.BooleanField(default=True)
    ...
    objects = UserSubsciptionManager()
    ...
    def serialize(self):
        return {
            ...
            "status": self.status,
            "currrent_period_start": self.current_period_start,
    ...
    def  billing_cycle_anchor(self):
        """
        Optional delay to start new subsciption in
        Stripe Checkout
        """
...
def user_sub_post_save(sender, instance, created, **kwargs):
    ...
    if subscription_obj is not None:
        ...
        groups_ids = set(groups.values_list('id', flat=True))
    if not ALLOW_CUSTOM_GROUPS:
        ...
        sub_groups = set(subs_qs.values_list('groups__id', flat=True))
        
        current_groups = set(user.groups.all().values_list('id', flat=True)) - sub_groups
        fin_group_ids = list(groups_ids | current_groups)
        user.groups.set(fin_group_ids)
