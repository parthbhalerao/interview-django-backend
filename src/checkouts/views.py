...
def checkout_redirect_view(request):
    ...
    success_url_path = reverse('stripe-checkout-success')
...
def checkout_success_view(request):
    ...
    if None in [sub_obj, user_obj, _user_sub_obj]:
        return HttpResponseBadRequest("Missing required data for subscription. Please contact us.")
    ...
    if _user_sub_exists:
        ...
        if old_stripe_id is not None and not same_stripe_id:
            try:
                helpers.billing.cancel_subscription(old_stripe_id, "User changed plan", feedback="other")
            except:
                pass
